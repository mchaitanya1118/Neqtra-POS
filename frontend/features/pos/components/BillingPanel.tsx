...
export function BillingPanel() {
    ...
    const fetchActiveOrder = async () => {
        ...
        try {
            const res = await apiClient.get(`/orders/${selectedTable.id}/active`, {
    ...
    const handleShiftTable = async (targetTableId: number) => {
        ...
        try {
            const res = await apiClient.post('/tables/shift', { fromId: selectedTable.id, toId: targetTableId });
            ...
            console.error(e);
            alert(`Shift Failed: ${e.response?.data?.message || e.message}`);
        } finally {
    ...
    const handleAction = async (action: 'SAVE' | 'PRINT' | 'KOT' | 'SETTLE') => {
        ...
        try {
            ...
            if (action === 'SETTLE' && currentOrder) {
                ...
                const settleRes = await apiClient.post(`/orders/${currentOrder.id}/settle`, { amount: remainingDue, method: paymentMode.toUpperCase() });
    ...
    return (
        <div className="flex flex-col h-full bg-surface-dark relative">
            ...
            {/* Header: Table / Customer Glass Pannel */}
            <div className="bg-surface/40 backdrop-blur-md border-b border-surface-light p-4 px-5 space-y-3">
                <div className="flex items-center justify-between">
                    {isDeliveryMode ? (
                        <div className="flex-1 mr-4 space-y-1.5">
                            <div className="flex items-center gap-2">
                                <Truck className="w-4 h-4 text-primary" />
                                <span className="text-[10px] font-bold text-primary uppercase tracking-widest">Delivery Mode</span>
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                                <input
                                    className="w-full bg-surface-light/50 border border-surface-light rounded-xl px-3 py-1.5 text-xs text-foreground placeholder:text-muted focus:outline-none focus:border-primary/50"
                        ...
                        >
                            <span className="text-[9px] font-bold text-muted uppercase tracking-[0.2em] group-hover:text-primary transition-colors">Station</span>
                            <div className="flex items-center gap-2 mt-0.5">
                                <span className="text-2xl font-bold font-serif italic tracking-tighter text-foreground group-hover:translate-x-1 transition-transform">{selectedTable?.label || '--'}</span>
                                <ChevronDown className="w-4 h-4 text-muted opacity-0 group-hover:opacity-100 transition-all" />
                                {selectedTable && selectedTable.status === 'OCCUPIED' && (
                                    <button
                                        ...
                                        className="ml-2 px-2.5 py-1 bg-primary text-primary-fg rounded-lg transition-all hover:scale-105 active:scale-95 flex items-center gap-1.5 shadow-lg shadow-primary/20"
                                        title="Shift Table"
                                    >
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className={cn(
                            "flex-1 flex items-center gap-2 px-4 py-2 rounded-xl border transition-all duration-300",
                    ...
                    >
                        <User className={cn("w-3.5 h-3.5", customer ? "text-primary" : "text-muted")} />
                        <span className="text-[11px] font-bold truncate">{customer?.name || 'Assign Customer'}</span>
                        {customer && <X className="w-3 h-3 ml-auto text-muted hover:text-destructive" onClick={(e) => { e.stopPropagation(); setCustomer(null); }} />}
                    </button>

                    <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-background/40 border border-surface-light">
                        <Users className="w-3.5 h-3.5 text-muted" />
                        <span className="text-[10px] font-bold text-muted uppercase tracking-wider">{user?.name || 'Staff'}</span>
                    </div>
                </div>
            </div>
                        ...
                        <div className="divide-y divide-surface-light/30">
                            ...
                            {items.map((item) => (
                                <motion.div
                                    ...
                                    key={item.menuItemId}
                                    className="py-3 px-5 flex items-center hover:bg-surface-light/20 group transition-all"
                                >
                                        ...
                                        <button
                                            onClick={() => {
                                                if (!hasPermission('Admin')) {
                                                    alert("Only Admins can modify placed items.");
                                                    return;
                                                }
            ...
            <div className="bg-surface/80 backdrop-blur-3xl border-t border-surface-light shrink-0 p-6 space-y-6">
                ...
                {/* Discount & Math */}
                <div className="flex items-center gap-4 bg-background/40 rounded-3xl border border-surface-light p-1.5 pr-6">
                    <div className="flex bg-surface-light rounded-2xl overflow-hidden p-1 gap-1">
                        <button
                            onClick={() => setDiscountType('FIXED')}
                            disabled={!hasPermission('Admin')}
                            className={cn("w-8 h-8 flex items-center justify-center rounded-xl text-xs font-bold transition-all disabled:opacity-30", discountType === 'FIXED' ? "bg-primary text-primary-fg shadow-lg shadow-primary/20" : "text-muted hover:text-foreground")}
                        >
                        ...
                        <button
                            ...
                            disabled={!hasPermission('Admin')}
                            className={cn("w-8 h-8 flex items-center justify-center rounded-xl text-xs font-bold transition-all disabled:opacity-30", discountType === 'PERCENT' ? "bg-primary text-primary-fg shadow-lg shadow-primary/20" : "text-muted hover:text-foreground")}
                        >
                            <Percent className="w-3.5 h-3.5" />
                        </button>
                    </div>
                    <input
                        className="flex-1 bg-transparent text-sm font-bold text-foreground placeholder:text-muted/30 focus:outline-none px-2 disabled:cursor-not-allowed"
                ...
                <div className="space-y-3">
                    <div className="flex justify-between items-center px-2">
                        <div className="flex gap-4">
                            <button onClick={() => setIsSplitOpen(true)} className="flex items-center gap-2 text-[9px] font-bold text-muted hover:text-foreground uppercase tracking-[0.2em] transition-colors">
                            ...
                            <button
                                onClick={() => {
                                    if (!hasPermission('Admin')) {
                                        alert("Only Admins can flush the registry.");
                                        return;
                                    }
                                    clearCart();
                                }}
                                className="flex items-center gap-2 text-[9px] font-bold text-muted hover:text-red-400 uppercase tracking-[0.2em] transition-colors"
                            >
                        ...
                        <div className="flex items-baseline gap-2">
                            <span className="text-[9px] font-bold text-muted uppercase tracking-widest">Registry Sync</span>
                            <span className="text-lg font-bold tracking-tighter">₹{grandTotal.toFixed(2)}</span>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {[
                            ...
                            <button
                                key={id}
                                onClick={() => {
                                    if (id === 'Due' && (!customer || !hasPermission('Admin'))) {
                                ...
                                className={cn(
                                    "flex flex-col items-center justify-center py-2.5 rounded-2xl border transition-all duration-300 gap-1",
                                    paymentMode === id
                ...
                <div className="grid grid-cols-12 gap-3 pt-1">
                    <button
                        ...
                        className={cn(
                            "py-3.5 bg-primary hover:bg-primary/90 disabled:opacity-30 text-primary-fg rounded-[32px] font-bold text-sm shadow-[0_10px_30px_rgba(105,215,189,0.3)] transition-all uppercase tracking-[0.3em] hover:scale-[1.02] active:scale-[0.98]",
                            existingOrder ? "col-span-8" : "col-span-12"
                        )}
                    >
                    ...
                    {existingOrder && (
                        <button
                            ...
                            disabled={isProcessing || !hasPermission('Billing')}
                            className="col-span-4 py-3.5 bg-surface-light hover:bg-surface text-foreground rounded-[32px] flex items-center justify-center border border-surface-light transition-all hover:scale-[1.05] active:scale-95"
                        >
                    ...
                    {(existingOrder || items.length > 0) && (
                        <button
                            ...
                            disabled={isProcessing || !hasPermission('Billing') || (items.length === 0 && !existingOrder)}
                            className="col-span-full py-3.5 bg-foreground text-background hover:bg-foreground/90 disabled:opacity-30 rounded-[32px] font-bold text-sm transition-all uppercase tracking-[0.4em] transform hover:scale-[1.01] active:scale-[0.99] shadow-2xl"
                        >
