...
export function SplitBillModal({ isOpen, onClose, tableId, onSuccess }: SplitBillModalProps) {
    ...
    const fetchOrder = async () => {
        try {
            const res = await apiClient.get(`/orders/${tableId}/active`);
            const data = res.data;
            setOrder(data);
    ...
    const handlePayment = async () => {
        if (!amount || Number(amount) <= 0 || !order) return;
        setLoading(true);
        try {
            const res = await apiClient.post(`/orders/${order.id}/settle`, { amount: Number(amount), method });
            const data = res.data;
