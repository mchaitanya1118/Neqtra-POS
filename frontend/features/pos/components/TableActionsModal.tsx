...
export function TableActionsModal({ isOpen, onClose, onShiftTable }: TableActionsModalProps) {
    ...
    useEffect(() => {
        if (isOpen && selectedTable) {
            apiClient.get(`/orders/${selectedTable.id}/active`)
    ...
    const handleSettleBill = async () => {
        ...
        setIsSettling(true);
        try {
            await apiClient.post(`/orders/${activeOrder.id}/settle`, {
                method: 'CASH', // Default to Cash from this shortcut
                amount: activeOrder.totalAmount - (activeOrder.totalPaid || 0)
            });
            ...
            console.error(e);
            alert(`Failed to settle: ${e.response?.data?.message || e.message}`);
        } finally {
            setIsSettling(false);
        }
    };
